...
> Every row's D4 review gate dispatches **`spoke-parity-checker`** subagent per ADR-0036 (functional parity comparison Spoke vs RotPro live via adb on M54) + **`prototype-fidelity-checker`** (visual tokens post-ADR-0035) + **`flutter-perf-auditor`** for screens with lists/maps/animations. The "Spoke deep-dive needed?" column below refers to the UPFRONT gate (does the spec author need to inspect Spoke before writing the spec) — the CLOSING gate is unconditional for every Spoke-aligned microsprint.

| Microsprint | Scope | Est. | Dependencies | Upfront Spoke deep-dive? |
|---|---|---|---|---|
...
| **MS-A8** | **Slice 2 release** — rebuild APK como `v1.1.0`, sweep visual de toda divergência pendente vs prototipo (per directive #1 "ajusta depois"), `prototype-fidelity-checker` (visual-only post-ADR-0035) sobre cada Dart file novo, `flutter-perf-auditor` sobre as 3 telas mais complexas (RoutesList, StopDetail expandida, MS-A4 settings), full-slice `spoke-parity-checker` re-run on golden path end-to-end as a regression check, `flutter analyze` + `flutter test` clean, `aapt2 dump permissions`, `apksigner verify`, E2E M54, PR `feat/m2-slice-2-telas-core` → `develop` → tag `v1.1.0`. | 2 d | MS-A1..A7 | n/a (release gate) |

**ADRs to file during slice 2** (numbering reserved post-ADR-0036 which is the parity gate ADR):
- ADR-0037: Route entity introduction (data model change; major impact)
- ADR-0038: Google Sign-In integration (auth provider choice + library — `google_sign_in` package; cite Context7)
- ADR-0039: Theme mode toggle (Material 3 dynamic theming + system follow)
...
> Same parity-gate policy as slice 2: every Spoke-aligned UI-facing microsprint (MS-B5, MS-B6, MS-B7, MS-B9) dispatches **`spoke-parity-checker`** at D4. Pure-backend microsprints (MS-B1, MS-B2, MS-B3, MS-B4, MS-B8) are exempt from the parity gate because they have no UI to compare side-by-side; they go through curl smoke + integration tests + standard backend verification instead.

| Microsprint | Scope | Est. | Dependencies | Upfront Spoke deep-dive? | D4 parity-gate? |
|---|---|---|---|---|---|
| **MS-B1** | **GraphHopper tag pinning + amendment** to ADR-0008. Pin from `:latest` to stable tag (likely `:9.1`), re-validate `config.yml graph.encoded_values`, rebuild graph-cache, `benchmark.sh` p95 < 200ms. Carried tech debt from session 2026-05-24-21. | 1 d | nothing | n/a | ❌ backend-only |
| **MS-B2** | **Backend auth additions.** Google Sign-In token verification endpoint (`POST /auth/google { idToken }`) returning JWT pair. Password reset flow (`POST /auth/forgot-password { email }` → email link → `POST /auth/reset-password { token, newPassword }`). Wire MS-A2 UI. | 2 d | MS-A2 (UI) | n/a | ❌ backend-only |
| **MS-B3** | **Solver + Nominatim self-hosted.** File **ADR-0040** (solver design: nearest-neighbor + 2-opt) + **ADR-0041** (Nominatim self-hosted SP-Capital, ~4GB RAM ~R$ 120/mês per cost model). Implement `apps/backend/src/routes/solver.ts` (~150 LOC TS, no deps). `POST /geocode` backed by Nominatim. Distance matrix via `Promise.all` GraphHopper calls + Redis cache (5min TTL). Replace 501 → real `POST /routes/optimize`. | 3-4 d | MS-B1 | n/a | ❌ backend-only |
| **MS-B4** | **Routes persistence (entity from MS-A1 goes to DB).** Prisma migration: `routes` + `route_stops` tables. Endpoints: `GET /routes` (list), `POST /routes` (create), `GET /routes/:id`, `PATCH /routes/:id`, `DELETE /routes/:id`. Mobile: swap `SharedPrefsRoutesRepository` (MS-A1) for `ApiRoutesRepository`. SharedPrefs becomes cache layer only. | 2 d | MS-A1 + MS-B3 | n/a | ❌ persistence swap, UI unchanged |
| **MS-B5** | **Stop status + notes + POD persistence.** Prisma migration: add `status`, `failure_reason`, `notes`, `pod_photo_url` columns to `stops`. POD photo upload endpoint (`POST /stops/:id/pod-photo` multipart → S3-compatible storage; DO Spaces or similar). Mobile: photo picker writes to backend; thumbnail download for history. Reutilizar paradas: `GET /routes/:id/copy-stops-to/:targetRouteId`. | 2-3 d | MS-A3 + MS-B4 | ⚠️ Spoke POD UI não inspecionado | ✅ POD UI side-by-side |
| **MS-B6** | **OCR multi-stop (manifesto) + multi-address dictation.** Voice flow gains "fale vários endereços" CTA (per inventory §3.2#10b — Spoke trata como first-class). Voice transcript segmentação por pauseFor + pontuação; cada segmento vira geocoding call separado + confirmação inline. OCR gains "ler manifesto" entry: foto de lista impressa → ML Kit text recognition → linhas viram candidatos de paradas + UI de confirmação batch. | 2-3 d | MS-B3 (geocoding) | ⚠️ Spoke OCR full + Voice multi não inspecionados | ✅ both flows side-by-side |
| **MS-B7** | **Importar manifesto (CSV/Excel) + Transferir paradas.** Importer: file picker → CSV/XLSX parser (`csv` + `excel` Dart packages — Context7 verify) → UI de mapeamento de colunas (endereço, complemento, observação) → batch geocoding → batch insert. Transferir paradas: source rota + target rota seletor. | 2-3 d | MS-B4 + MS-B6 | ⚠️ Spoke importer UI não inspecionado | ✅ both flows side-by-side |
| **MS-B8** | **FCM push notifications.** Firebase setup (free tier 1M/msg). Backend: `firebase-admin` SDK + `POST /notifications/send` internal endpoint + cron triggers (lembrete início de rota X min antes do horário planejado). Mobile: `firebase_messaging` Flutter pkg, handler de notification → deep link pra tela relevante. Wire MS-A7 preferences. | 2 d | MS-A7 + MS-B4 | ❌ FCM é nosso — sem deep-dive | ❌ infra-only, UI is MS-A7 |
| **MS-B9** | **Histórico de rotas + métricas básicas.** Endpoint `GET /routes/history?period=...` retorna lista paginada com totais (paradas, entregues, falhas, km, tempo). Tela mobile `RoutesHistoryPage` (acessível via tab Rota → "Histórico" ou direto da `RoutesListPage`). | 1-2 d | MS-B4 + MS-B5 | ⚠️ Spoke histórico não inspecionado | ✅ history UI side-by-side |

**ADRs to file** (renumbered post-ADR-0036 parity gate): ADR-0008 amendment (MS-B1) + ADR-0037/0038/0039 (slice 2) + ADR-0040 solver + ADR-0041 Nominatim hosted + possivelmente ADR-0042 FCM setup + ADR-0043 importer parser strategy.
