...
- [ ] **HARD GATE — device parity sweep (ADR-0021 + ADR-0035).** Before PR open, every screen exercised in the device E2E above must (a) match Spoke's behavior for the corresponding flow per `docs/inventory/2026-05-26-spoke-vs-rotpro.md` AND (b) use the visual identity tokens from `prototipo/` (colors, spacing, radii, shadows, typography, Lucide icon family). The session-17 slice-2 audit proved that `flutter analyze` + `flutter test` + static `prototype-fidelity-checker` can pass while the device still ships behavioral or visual Criticals. The fix: capture a screenshot per E2E step under `docs/sessions/<slice-session>/screenshots/`; either (a) eyeball against Spoke + tokens or (b) dispatch `prototype-fidelity-checker` (visual-only post-ADR-0035) on each touched Dart file plus a manual Spoke-parity check using the inventory. Any Critical found here blocks the tag.
