...
> **Last updated:** 2026-06-11 (harness dump-first hardening: `warn-dump-first.sh` agora também avisa **edits em `lib/features/**` sem consulta ao dump** — o failure mode implement-from-inference que o trigger original não via; `/verify-slice` dispatcha `spoke-parity-checker` **D4 dump-only** per ADR-0049 no lugar do `prototype-fidelity-checker` [polish-final-only]; `spoke-parity-checker` Rule 1 reescrita **Dump > Docs** [NUNCA websearch p/ comportamento Spoke] + modo D4 dump-only codificado no agent file. Previous 2026-06-10: ADR-0048 — dump-first vira gate signal-only: hook `warn-dump-first.sh` (Stop) avisa runtime/websearch sem dump antes + SessionStart re-injeta a regra; websearch/Context7 só para libs, não para comportamento Spoke. ADR-0046 config-summary na tela ativa + ADR-0047 persistência route-defaults (FTUE cortado, dump provou que a Spoke não tem gate de 1ª rota). ADR-0045 — dump estático completo do Spoke v3.65.1 como baseline funcional; `docs/inventory/spoke-dump-v3.65.1/MASTER-TABLE.md` substitui paráfrase por fato e INVERTE o método: dump-first → runtime-confirm. ADR-0044 Pausa = página full-screen + janela de horário. Previous 2026-06-06: drift sweep — `docs/08-ROADMAP-v2.md` reescrito limpo como fonte única; ADR-0042 numpad + ADR-0043 Destino 3-card sheet. Previous 2026-05-27: engineering artifacts da inspeção podem entrar no repo livremente; inspeção é escolha do operador; o que rege é o **shipped product** ter identidade visual original per ADR-0035. Foundational: ADR-0035 pivot + ADR-0036 parity gate + ADR-0037 Maestro MCP.)
...
- Use `/verify-slice` as the pre-PR gate — it packages `M2-SLICE-CHECKLIST.md` §Verification (flutter analyze + test, bun typecheck, `spoke-parity-checker` D4 **dump-only** dispatch per ADR-0049 — skipped for Á1/Á11 — plus `adr-guardian`) into one orchestrated report. `prototype-fidelity-checker` is NOT in this gate (final polish only). See ADR-0018; gate composition updated 2026-06-11.
...
- `warn-dump-first.sh` (ADR-0048, extended 2026-06-11) — warns on TWO triggers: (a) the session ran live Spoke runtime inspection (Maestro/`adb`/`uiautomator`) or a `WebSearch` **without** consulting the static dump baseline first; (b) the session **edited `apps/mobile/lib/features/**`** (except `features/auth/` — Á1) without any dump-derived consultation at all — the implement-from-inference failure mode (ADR-0041..0044) that trigger (a) alone can't see. Accepted consultations: `MASTER-TABLE.md` Read, a `~/spoke-dump`/`jadx-out` grep, or a `docs/superpowers/specs/*-design.md` Read (distilled dump baselines). Signal-only; mirrors `warn-adr-drift`. Exceptions: Áreas without a Spoke baseline (Á1, Á11), pure bugfix/refactor/test sessions, genuinely-new libraries.
