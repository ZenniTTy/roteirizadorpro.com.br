...
description: Use ONLY in the final visual-polish pass (roadmap §"Polish visual final", after Slices 2–7 are functionally complete) — NOT during slice implementation and NOT as part of /verify-slice or any per-PR gate (per M2-SLICE-CHECKLIST "Sem prototype-fidelity-checker durante implementação"; the per-slice gate is spoke-parity-checker). Verifies that shipped Flutter UI in apps/mobile/ uses the visual identity tokens from prototipo/ — compares colors, spacing, radii, shadows, typography, and icon family against prototipo/tokens.js (and component reference in prototipo/ui.jsx for token-anchored values). Reports visual-identity divergences as a punch list — does not edit code, does not check screen structure or flow (those are Spoke-aligned per ADR-0035 and tracked in docs/inventory/2026-05-26-spoke-vs-rotpro.md). Trigger only when the user explicitly says "review the UI tokens", "check visual identity", "polish sweep", or the final-polish phase is active.
