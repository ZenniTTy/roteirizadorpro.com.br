...
description: Use proactively at the D4 review gate of every slice-2 (Spoke-aligned Telas Core) and slice-3 (Real backend) microsprint, BEFORE opening the slice PR. Inspects the reference app (`com.underwood.route_optimiser` on the connected M54 device) live via adb uiautomator dump + screencap for a named flow, inspects the Roteirizador Pro equivalent flow the same way, and produces a categorized punch list of behavioral / structural gaps (must-fix / should-fix / nit). Read-only — does not edit code, does not run tests. Trigger when a microsprint finishes its green pass and is ready for D4, or when the user says "spoke check <flow>" / "parity check <flow>".
